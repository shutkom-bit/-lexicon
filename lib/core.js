...
Use standard, natural, uncontracted forms (e.g. "will work", not "'ll work"). If the verb is irregular, use its correct irregular forms.`;
  ...
  // Явний таймаут коротший за ліміт виконання функції на Vercel (Hobby ~10с) —
  // якщо DeepSeek не встигає, отримуємо чітку помилку сюди, а не мовчазне
  // "зависання" запиту на боці клієнта через примусове вбивство функції Vercel.
  ...
  try {
    ...
    if (e && e.name === 'AbortError') throw new Error('LLM request timed out after 8s');
    throw e;
  } finally {
