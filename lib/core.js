...
const LANG_NAMES = {
  ...
  es: 'Spanish', it: 'Italian', pl: 'Polish', de: 'German', ru: 'Russian'
};
...
If it is a multi-word expression, set pos_group to "phrase".`;
  ...
  // AbortSignal.timeout — вбудований у сам fetch механізм, керується мережевим
  // шаром, а не окремим JS-таймером (той не завжди надійно спрацьовує в
  // serverless-середовищах на кшталт Vercel Fluid Compute під час очікування
  // мережевого запиту). Якщо DeepSeek зависає — падаємо чітко за 20с, а не
  // чекаємо, поки Vercel вб'є всю функцію примусово через 300с.
  let r;
  try {
    ...
    if (e && e.name === 'TimeoutError') throw new Error('LLM request timed out after 20s');
    throw e;
  }
  ...
  // Якщо модель повернула валідний JSON, але практично без вмісту (жодного
  // перекладу, означення чи прикладу) — це фактична невдача збагачення, а не
  // "порожній, але успішний" результат. Кидаємо помилку, щоб виклик отримав
  // те саме чітке "не вдалося", що й при тайм-ауті — а не тихо порожнє слово.
  const hasAnyTranslation = natives.some(c => tr[c]);
  if (!hasAnyTranslation && !parsed.definition && !parsed.example) {
    throw new Error('LLM returned an empty enrichment (no translation/definition/example)');
  }
...
const TTS_LANG_BCP47 = {
  ...
  es: 'es-ES', it: 'it-IT', pl: 'pl-PL', de: 'de-DE', ru: 'ru-RU'
};
...
Use standard, natural, uncontracted forms (e.g. "will work", not "'ll work"). If the verb is irregular, use its correct irregular forms.`;
  ...
  // AbortSignal.timeout — вбудований у fetch механізм (не самописний JS-таймер,
  // який виявився ненадійним у serverless-середовищі з Fluid Compute під час
  // очікування мережевого запиту). Якщо DeepSeek зависає — падаємо чітко за
  // 20с, а не чекаємо примусового вбивства функції Vercel через 300с.
  let r;
  try {
    r = await fetch(`${BASE}/chat/completions`, {
      method: 'POST',
